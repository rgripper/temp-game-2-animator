...
function FramePose({ pose }: { pose: Pose }) {
  ...
  console.log({
    characterDirection,
    right: keypointMap.right_ear.x,
    nose: keypointMap.nose.x,
    left: keypointMap.left_ear.x,
  });
  return (
    <svg className={bodyStyles} viewBox="0 0 800 800" width="500" height="500" xmlns="http://www.w3.org/2000/svg">
      <Head keypointMap={keypointMap} characterDirection={characterDirection} />
...
function getHeadX(keypointMap: KeypointMap, characterDirection: CharacterDirection, width: number) {
  switch (characterDirection) {
    case 'left':
      return keypointMap.nose.x;
    case 'right':
      return keypointMap.nose.x + width;
    case 'front':
    case 'back':
      return keypointMap.nose.x - width / 2;
  }
}
...
function Head({
  ...
  return <rect x={midX - width / 2} y={keypointMap.nose.y - width / 2} width={width} height={width} fill="blue"></rect>;
}
